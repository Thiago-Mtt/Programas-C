/*
Implementação da função strtod, da biblioteca stdlib.h
...
Altera a posição do ponteiro, posiciona-o no primeiro caracter após o segmento correspondente
    ao número de ponto flutuante
Adaptado de func_atof.c
*/
...
double func_strtod(char *string_float, char **string_end){
    printf("string_float na função = %s\n", string_float);
    printf("string_end na funçaõ = %c\n", *string_end);
...
int main (){
    char string[] = "12.3241ABC";
    char *p;
    printf("numero float = %f\n", func_strtod(string, &p));
    printf("Texto do ponteiro retornado = %s\n", p);
    return(0);
}